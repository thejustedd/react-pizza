...
const AddButton: FC<AddButtonProps> = ({ pizza, activeTypeIndex, activeSizeIndex }) => {
  const cartItem = useAppSelector(selectCartItemById(pizza.id));
  ...
  function addToCart() {
    const item = {
      ...
      type: typeNames[activeTypeIndex ?? 0],
      size: pizza.sizes[activeSizeIndex ?? 0],
    };
